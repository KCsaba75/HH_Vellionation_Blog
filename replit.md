...
- **Critical CSS & LCP Optimization**:
  - Created src/critical.css with above-the-fold styles (~6KB minified)
  - Vite plugin inlines critical CSS into index.html during build
  - Main CSS (70KB) now loads async via preload+onload pattern
  - Includes CSS variables, header, hero, button, and layout utilities
  - Eliminates render-blocking CSS for faster LCP
- **Site Image Management**:
  ...
  - No hardcoded fallback images - shows English placeholder text when no image uploaded:
    - Header: "Logo placeholder" in circular container
