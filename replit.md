...
- **Font Loading & CLS Mitigation**: Google Fonts (Inter + Poppins, the families referenced by `src/index.css` body and headings) are loaded from `index.html` with `preconnect` to `fonts.googleapis.com` + `fonts.gstatic.com`, a `rel="preload" as="style"` hint for the CSS, and the stylesheet loaded non-render-blocking via the `media="print" onload="this.media='all'"` trick (with a `<noscript>` fallback). The Google Fonts URL uses `&display=swap` so text renders immediately with the system fallback and swaps in once Inter/Poppins arrive — preventing FOIT and minimizing late layout shifts. These tags sit AFTER the `<!--PRERENDER:SEO-->…<!--/PRERENDER:SEO-->` block so the prerender script preserves them on every prerendered route. To prevent additional CLS from async-loaded header content, the social-icon container in `src/components/Header.jsx` reserves `min-w-[120px] min-h-[24px]` so when `getSettings()` resolves and the Facebook/Instagram/YouTube/Spotify icons render, they fill pre-reserved space instead of pushing surrounding flex content. Existing CLS guards (DailyTipBanner `min-h-[88px]`, hero image `aspectRatio: '800/533'` wrapper + width/height attrs, header logo fixed `w-16 h-16` slot) are unchanged.
